...
namespace SevenTiny.Cloud.MultiTenantPlatform.Domain.ServiceContract
{
    public interface IFieldAggregationService : IRepository<FieldAggregation>
    {
        ...
        /// <returns></returns>
        List<MetaField> GetMetaFieldsByInterfaceFieldId(int interfaceFieldId);
        /// <summary>
        /// 获取接口配置的字段信息通过接口字段配置id,返回字典形式
        /// </summary>
        /// <param name="interfaceFieldId"></param>
        /// <returns></returns>
        Dictionary<string, MetaField> GetMetaFieldsDicByInterfaceFieldId(int interfaceFieldId);
    }
}
